...
public class UpgradeCapture : Capture
{
    ...
    private string getNewName(string currName, Dictionary<string, int> pieceNum)
    {
        ...
        {
            if (!pieceNum.ContainsKey(currName[0].ToString() + "Q"))
            {
                pieceNum.Add(currName[0].ToString() + "Q", 1);
            }
            newName = currName[0].ToString() + "Q" + (pieceNum[currName[0].ToString() + "Q"] + 1).ToString();
        }
        else if (currName[1] == 'Q' || currName[1] == 'K')
        {
            newName = currName;
        }
        return newName;
