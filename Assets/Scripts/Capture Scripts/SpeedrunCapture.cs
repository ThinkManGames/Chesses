...
public class SpeedrunCapture : Capture
{
    public bool canMakeAMove = true;

    public override string[,] movementCheck(SpotBehavior spot, string[,] board, int selectedRow, int selectedCol)
    {
        if (spot == null || selectedRow == -1 || selectedCol == -1)
        {
            return board;
        } else
        {
            return base.movementCheck(spot, board, selectedRow, selectedCol);
        }
    }
