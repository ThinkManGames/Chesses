...
public class NoRetreatMoves : Moves
{
    public override int[,] possibleMoves(char color, string pieceName, int row, int col, string[,] board)
    {
        ...
        int[,] onlyCaptureBoard = new int[tRow, tCol];
        bool onlyCapture = false;
        ...
        switch (toSwitch)
        {
            case 'P':
                if (color == 'W')
                {
                    if (row < tRow - 1)
                    {
                        ...
                        {
                            onlyCapture = true;
                            onlyCaptureBoard[row + 1, col - 1] = 1;
                        }
            ...
            case 'N':
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    {
                        if(board[row + knightMoves[i, 0], col + knightMoves[i, 1]][0] != 'E')
                        {
                            onlyCapture = true;
                            onlyCaptureBoard[row + knightMoves[i, 0], col + knightMoves[i, 1]] = 1;
                        }
            ...
            case 'B':
                for (int i = 0; i < 4; i++)
                {
                    ...
                    while (currRow >= 0 && currRow < tRow && currCol >= 0 && currCol < tCol && board[currRow, currCol][0] != color && board[currRow, currCol] != "0")
                    {
                        ...
                        {
                            onlyCapture = true;
                            onlyCaptureBoard[currRow, currCol] = 1;
                            break;
                        }
                        currRow += rowAdder;
                        currCol += colAdder;
