...
public class PiercingWin : Win
{
    public override string getDangerousSpots(char color, int _row, int _col, string[,] board)
    {
        ...
        int[,] knightMoves = world.moves.possibleMoves(otherColor, "DN1", _row, _col, board);
        int[,] rookMoves = world.moves.possibleMoves(otherColor, "DR1", _row, _col, board);
        int[,] bishopMoves = world.moves.possibleMoves(otherColor, "DB1", _row, _col, board);
        int[,] kingMoves = world.moves.possibleMoves(otherColor, "DK1", _row, _col, board);
